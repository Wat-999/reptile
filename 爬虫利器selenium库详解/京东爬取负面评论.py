#通过Network寻找真正的评论网址
#如果只想爬取负面评价，可以用前面相同的方法，单击页面上的"差评"按钮后，用开发者工具进行分析，获取差评的数据接口对比第一页的数据接口会发现
#唯一的区别就是参数score的值由5变成1，因此只需把前面代码中网址的参数score由5改成1，即可爬取差评。
import requests
import re
headers = {
...
for i in range(10):
    url = 'https://club.jd.com/comment/productPageComments.action?callback=fetchJSON_comment98&productId=10048802240823&score=1&' \
